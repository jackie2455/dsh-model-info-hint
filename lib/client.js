  ...
  factory: (require) => {
    ...
    function resolveHover(target) {
      ...
      if (menuitem !== null) {
        ...
        // The menu popup is the surface we keep the hint clear of.
        const surface = menuitem.closest('[role="menu"]');
        return { groupName, modelName, item: menuitem, surface };
      }
      ...
      if (option !== null) {
        ...
        const surface = option.closest('[role="listbox"]');
        return { groupName, modelName, item: option, surface };
      }
    ...
    /**
     * Place the hint outside the model list whenever possible, so it does not
     * cover the other candidate models: prefer the side (left/right) of the
     * list surface that has the most free room, vertically centered on the
     * hovered item and clamped to the viewport. Falls back to cursor-relative
     * placement only when no list surface could be detected.
     */
    function positionTip(event, surfaceRect, itemRect) {
      const vw = window.innerWidth;
      const vh = window.innerHeight;

      if (surfaceRect !== null) {
        const spaceRight = vw - MARGIN - (surfaceRect.right + GAP);
        const spaceLeft = surfaceRect.left - GAP - MARGIN;
        let left =
          spaceRight >= spaceLeft
            ? surfaceRect.right + GAP
            : surfaceRect.left - TIP_W - GAP;
        left = Math.min(Math.max(left, MARGIN), vw - TIP_W - MARGIN);

        const anchorY = itemRect !== null ? itemRect.top + itemRect.height / 2 : event.clientY;
        const top = Math.min(Math.max(anchorY - TIP_H / 2, MARGIN), vh - TIP_H - MARGIN);
        return { left, top };
      }

      let left = event.clientX + GAP;
      let top = event.clientY + GAP;
      if (left + TIP_W > vw - MARGIN) left = event.clientX - TIP_W - GAP;
      if (top + TIP_H > vh - MARGIN) top = vh - TIP_H - MARGIN;
    ...
    function ModelInfoHint() {
      ...
      react.useEffect(() => {
        const onOver = (event) => {
          ...
          const surfaceRect = hit.surface !== null ? hit.surface.getBoundingClientRect() : null;
          const itemRect = hit.item !== null ? hit.item.getBoundingClientRect() : null;
          const { left, top } = positionTip(event, surfaceRect, itemRect);
          setTip({ row, left, top });
